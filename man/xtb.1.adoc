...
angle. For every coordinate, the output reports the diagonal compliance
constant C~ii~ and its reciprocal 1/C~ii~, the relaxed force constant. Bond
rows also report a local-mode frequency calculated from 1/C~ii~ and the bond
reduced mass.

Units depend on the coordinate type. For a bond, C~ii~ is in a0^2^/Eh and
1/C~ii~ is in Eh/a0^2^ (1 Eh/a0^2^ = 15.570 N/cm); angular coordinates and
mixed-coordinate couplings have their corresponding coordinate-dependent
units, so the bond conversion does not apply to them.
...
*compliance.dat*::
   compliance analysis from an eligible Hessian run. Lists every internal
   coordinate with C~ii~ and 1/C~ii~, followed by up to 20 strongest
   off-diagonal couplings per coordinate in descending order of magnitude.
