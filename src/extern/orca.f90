...
!> setup environment for ORCA run
   subroutine checkOrca(env, ext)
      ...
      ! GNOME screen reader is also called orca, check if this is the correct executable
      if (index(ext%executable, '/usr') == 1) then
         if (.not. isQCOrca(ext%executable)) then
            call env%error("Executable '"//ext%executable//"' is in the system path "//&
            & "and does not contain 'SCF'. Please make sure this is ORCA!", source)
            return
         end if
      end if
   ...
   contains
      function isQCOrca(filepath) result(isQC)
         ...
         do
            ...
            if (index(buffer, trim(keyword)) > 0) then
               isQC = .true.
               exit ! Keyword found, no need to read further.
            end if
         end do
