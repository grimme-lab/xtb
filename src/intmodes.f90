...
pure subroutine bonded(i,na,nb,nc,ind,n,molvec)
   ...
   if(nb(i) == 0)then
      do j=is+1,n
         icon=ind(j,i)
         if(icon.lt.i .and. icon.ne.na(i)) then
            is=j
            nb(i)=icon
            exit
         endif
...
!           tmp2(1:3,m)=xyzref(1:3,i,s1)                 ! copy bad zmat
!        enddo
...
!           m=m+1
!           tmp (1:3,m)=xyzref(1:3,i,1)
!           tmp2(1:3,m)=xyzref(1:3,i,s2)                 ! copy bad zmat
!        enddo
