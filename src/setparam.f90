...
!> Initialize the Fortran pseudorandom number generator from one scalar seed.
!>
!> If `$seed` supplied a value, that value is expanded directly.  `$samerand`
!> uses 41 as its legacy fixed scalar.  Otherwise, the processor
!> first initializes its RNG normally and xTB derives a scalar from the first
!> word of that state.  xTB then deliberately replaces the processor state
!> with the deterministic expansion of the scalar.  Consequently, the scalar
!> printed in the calculation setup contains all information needed to recreate
!> the state rather than merely describing part of an irrecoverable state.
!>
!> The scalar is retained in `set%randseed`, so later calls initialize the same
!> state instead of selecting new entropy.  Reusing the scalar reproduces the
!> sequence for the same compiler, runtime, and thread configuration; Fortran
!> does not standardize the algorithm behind `random_number`, so sequences can
!> differ across compilers.
subroutine initrand
   ...
   if (.not.set%randseed_set) then
      if (set%samerand) then
         ...
         ! Request the processor's normal initialization.  Reduce one state
         ! word to the range accepted by expand_random_seed; the full processor
         ! state is not used directly because a user could not replay it through
         ! the scalar `$seed` interface.
         call random_seed()
   ...
   call expand_random_seed(set%randseed, seed_vector)
   call random_seed(put=seed_vector)
end subroutine initrand

!> Expand one user-facing seed into the processor-dependent seed vector.
!>
!> `random_seed` requires a processor-dependent number of default integers
!> (for example, GFortran and ifx request different vector lengths).  A
!> Park--Miller recurrence produces a stable prefix of positive 31-bit values,
!> so every compiler receives as many deterministic words as it requests.
!> Positive values are used for compatibility with processors which reject
!> negative seed words.
!>
!> @param[in] seed Scalar value read from `$seed` or generated by `initrand`.
!> @param[out] seed_vector Expanded state with the size requested by the
!> processor's `random_seed(size=...)` implementation.
pure subroutine expand_random_seed(seed, seed_vector)
   use, intrinsic :: iso_fortran_env, only : int64
   implicit none
   integer, intent(in) :: seed
   integer, intent(out) :: seed_vector(:)
   integer :: i
   integer(int64) :: state
   integer(int64), parameter :: seed_modulus = 2147483647_int64
   integer(int64), parameter :: seed_multiplier = 48271_int64

   state = modulo(int(seed, int64), seed_modulus - 1_int64) + 1_int64
   do i = 1, size(seed_vector)
      state = modulo(seed_multiplier*state, seed_modulus)
      seed_vector(i) = int(state)
   enddo
end subroutine expand_random_seed
