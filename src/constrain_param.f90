...
subroutine set_wall(env,key,val,nat,at,idMap,xyz)
   ...
   !  part 1: get the sandwich distance
      ...
      center = 0.0_wp
      if (trim(argv(1)).eq.'auto') then
              radius=maxval(xyz(3,:))-minval(xyz(3,:)) !no need to set $cma in xcontrol, done automatically
      else
         if (getValue(env,trim(argv(1)),ddum)) then
            radius = ddum !in Bohr!!!
         else
      ...
      call set_sphere_radius(radius,center)
      write(env%unit,'("sandwich wallpotential with distance + 8A safety buffer",'//&
         '1x,f12.7,1x,"Å")') radius*autoaa+8.0_wp  !4A per side safety buffer
