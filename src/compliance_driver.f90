...
subroutine compliance_driver(unit, n, at, xyz, hess, mass)
   ...
   call compute_compliance(unit, hess, bmat, xyz, n, nint, compl, istat)
   if (istat /= 0) return
...
!> Prints diagonal elements of the compliance matrix in the order of bonds, angles, dihedrals,
!> and linear bends. Also calls write_compliance_dat to output the full compliance matrix to a file.
...
!> Writes the full compliance matrix to a file named "compliance.dat".
...
!> Build hauman-readable labels for internal coordinates
