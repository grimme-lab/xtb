...
subroutine gfnff_ini(env,pr,makeneighbor,mol,gen,param,topo,neigh,efield,accuracy,print_setup)
...
!--------------------------------------------------------------------------------------------------
      ...
      setup_print = .true.
      if (present(print_setup)) setup_print = print_setup
      pr_local = pr .and. setup_print
      ...
      if (setup_print) write(env%unit,'(10x,"Pauling EN used:")')
      do i=1,103
         if (setup_print .and. niel(i) .gt. 0) write(env%unit,'(10x,"Z :",i2,"  EN :",f6.2)') i,param%en(i)
      enddo

      dum = sqrt(sum(efield**2))
      if (setup_print) write(env%unit,'(10x,"electric field strengths (au):",f6.3)') dum
!     alp = alp *(1.+0.0*dum)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (setup_print) write(env%unit,'(10x,"distances ...")')
      pbo   = 0
      rab   = 0
      sqrab = 0
      !Get all distances
      ...
      do i=1, mol%n
        ...
        do j=1, i-1 
          ...
          if(rab(k).lt.1.d-3) then
            if (setup_print) write(env%unit,*) i,j,ati,atj,rab(k)
...
!  do the loop only if factor is significant
   do while ((qloop_count.lt.2.and.gen%rqshrink.gt.1.d-3).or.adjLnAn)
      ...
      call gfnff_topo_changes(env, neigh, print_setup=setup_print)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (setup_print) write(env%unit,'(10x,"pair mat ...")')
      ...
      if (setup_print) write(env%unit,'(10x,"computing topology distances matrix with Floyd-Warshall algo ...")')
...
!     they are used in the EEQ to determine qa (approximate topology charges)
      ...
      if (setup_print) write(env%unit,'(10x,"making topology EEQ charges ...")')
...
!     first check for fragments 
      ...
      if (setup_print) write(env%unit,'(10x,"#fragments for EEQ constrain: ",i0)') topo%nfrag
!     read QM info if it exists
      ...
      if (ich /= -1) then
         ...
         if (err == 0) then
            if (i < mol%n .or. abs(sum(qtmp) - mol%chrg) > 1.0e-3_wp) then
               ...
               if (setup_print) write(env%unit,'(10x,"fragment charges from <charges> :",10F7.3)') topo%qfrag(1:topo%nfrag)
            end if
         else
      ...
      if(mol%n.lt.100.and.topo%nfrag.gt.2.and.mol%chrg.ne.0.and.sum(topo%qfrag(2:topo%nfrag)).gt.999) then
         ...
         enddo
         do i=1,topo%nfrag
            if (setup_print) write(env%unit,*)i,itmp(i)
         enddo
      ...
      if(topo%nfrag.eq.2.and.mol%chrg.ne.0.and.sum(topo%qfrag(2:topo%nfrag)).gt.999) then
         if (setup_print) write(env%unit,*) 'trying auto detection of charge on 2 fragments:'
         topo%qfrag(1)=0
         ...
         if (setup_print) write(env%unit,'(10x,"#fragments for EEQ constrain from pdb file: ",i0)') topo%nfrag
...
if (mol%npbc.ne.0) then
  ...
  enddo
      ...
      if (setup_print) write(env%unit,'(10x,"rings ...")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if(topo%nbatm.gt.idum) then
         if (setup_print) write(env%unit,*) idum,topo%nbatm
         call env%error('overflow in ini', source)
         return
      endif
      if (setup_print) write(env%unit,'(10x,"# BATM",3x,i0)') topo%nbatm
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      enddo
      if (setup_print) write(env%unit,'(10x,"# H in HB",3x,i0)') topo%nathbH
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (setup_print) write(env%unit,'(10x,"doing iterative Hueckel for ",i0," subsystem(s) ...")') picount
      ...
      if (pr_local) then
         if (setup_print) write(env%unit,'(10x,"iterative Hueckel run to get P ...")')
      endif
      do pis=1,picount ! loop over pi systems
      npi   =0
      nelpi =0
...
! end of iterative loop
     ...
     &         3x, ''eps(HOMO/LUMO)'',2f12.6)')pis,ipis(pis),npi,nelpi,pisip(pis),pisea(pis)
      ...
      if(pisip(pis).gt.0.40) then
         if (setup_print) write(env%unit,'(a,i0,a)')'WARNING: probably wrong pi occupation for system ',pis,'. Second attempt with Nel=Nel-1!'
         do i=1,mol%n
            if (setup_print .and. piadr4(i) .ne. 0) write(env%unit,*) 'at,nb,topo%hyb,Npiel:', i,mol%sym(i),sum(neigh%nb(neigh%numnb,i,:)),topo%hyb(i),piel(i)
         enddo
         nelpi=nelpi-1
         Api = Apisave
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do i=1,mol%n
         ...
         if (setup_print) write(env%unit,'(i5,2x,a2,3x,i4,3x,f5.2,2x,f5.2,8x,i2,3x,i2,3x,i2,2x,f6.3,3f12.6)') &
...
! output
         ...
         if (pr_local) write(env%unit,'(2a3,2i5,2x,2i5,2x,6f8.3)') &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (setup_print) write(env%unit,'(10x,"#angl  :",3x,i0)') topo%nangl
      if (pr_local) then
         write(env%unit,*)
...
!              central*neigbor charge spec. met.  small angle corr.
               ...
               if (pr_local)write(env%unit,'(3i5,2x,3f8.3,l2,i4)') ii,jj,kk,r0,phi*180./pi,topo%vangl(2,topo%nangl),picon,rings
              enddo
            enddo
          enddo
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      enddo
      maxtors=topo%ntors
      if (pr_local) write(env%unit,*) 'torsion atoms        nrot   rings    phi0    phi      FC'
...
!                 printout
                  ...
                  if (pr_local)write(env%unit,'(4i5,2x,i2,5x,i2,4x,3f8.3)') &
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      if (pr_local) write(env%unit,*) 'out-of-plane atoms          phi0    phi      FC'
      do i=1,mol%n
...
!        printout
         ...
         if (pr_local)write(env%unit,'(4i5,7x,3f8.3)') i,jj,kk,ll,topo%vtors(1,topo%ntors)*180./pi,phi*180./pi,topo%vtors(2,topo%ntors)
      enddo
...
! all done
      ...
      if (setup_print) write(env%unit,'(10x,"#optfrag :",3x,i0)') topo%nfrag
...
subroutine gfnff_topo_changes(env, neigh, print_setup)
   use xtb_type_environment, only : TEnvironment
   ...
   if(allocated(set%ffnb)) then
      ...
      if (do_print) write(env%unit,*) 'The neighborlist has been adjusted according to the input file.'
      if (do_print) write(env%unit,*) ''
   end if
