...
!--------------------------------------------------------------------------------------------------
      ...
      allocate( rab(n*(n+1)/2), source = 0.0d0 )    
      allocate( cn(n), source = 0.0d0 )
      allocate( sqrab(n*(n+1)/2), source = 0.0d0 )
      allocate( hyb(n), source = 0 )
      allocate( alphanb(n*(n+1)/2), source = 0.0d0 )
      allocate( rtmp(n*(n+1)/2), source = 0.0d0 )
      allocate( pbo(n*(n+1)/2), source = 0.0d0 )
      allocate( piadr(n), source = 0 )
      allocate( piadr2(n), source = 0 )
      allocate( bpair(n*(n+1)/2), source = 0 )
      allocate( itmp(n), source = 0 )
      allocate( itag(n), source = 0 )
      allocate( sring(20,n), source = 0 )
      allocate( cring(10,20,n), source = 0 )
      allocate( piadr3(n), source = 0 )
      allocate( piadr4(n), source = 0 )
      allocate( qtmp(n), source = 0.0d0 )
      allocate( dxi(n), source = 0.0d0 )
      allocate( dgam(n), source = 0.0d0 )
      allocate( chieeq(n), source = 0.0d0 )
      allocate( gameeq(n), source = 0.0d0 )
      allocate( alpeeq(n), source = 0.0d0 )
      allocate( qa(n), source = 0.0d0 )
      allocate( dqa(n), source = 0.0d0)
      allocate( qah(n), source = 0.0d0 )
      allocate( nbm(20,n), source = 0 )
      allocate( mchar(n), source = 0.0d0 )
      allocate( imetal(n), source = 0 )
      allocate( zetac6(n*(n+1)/2), source = 0.0d0 )
      allocate( xyze0(3,n), source = 0.0d0 )
      allocate( nbf(20,n), source = 0 )
...
!     Calculate CN and derivative      
      allocate(dcn(3,n,n), source = 0.0d0 )
      call gfnff_dlogcoord(n,at,xyz,rab,cn,dcn,cnthr) ! dcn needed
      do i=1,n
         dum2=0
         do j=1,n
            dum2=dum2+sqrt(dcn(1,j,i)**2+dcn(2,j,i)**2+dcn(3,j,i)**2)
         enddo
         mchar(i) = exp(-0.005d0*en(at(i))**8)*dum2/(cn(i)+1.0d0)     ! estimated metallic character as ratio of av. dCN and CN
...
!  do the loop only if factor is significant
   do while (qloop_count.lt.2.and.rqshrink.gt.1.d-3)
      ...
      call gfnff_neigh(makeneighbor,n,at,xyz,rab,rqshrink,rthr,rthr2,linthr,mchar,hyb,itag,nbm,nbf) 

      do i=1,n
         imetal(i)=metal(at(i))
         if(nb(20,i).le.4.and.group(at(i)).gt.3) imetal(i)=0 ! Sn,Pb,Bi, with small CN are better described as non-metals
      enddo
...
! setup list of all possible pi atoms
      ...
      do i=1,n ! setup loop
         ...
         if(at(i).eq. 7.and.nb(20,i).gt.3) cycle           ! NR3-X is not a pi
...
!           ki=nb(2,i)
!           if(at(ki).eq.8.and.at(ji).eq.8.and.nb(20,ji).eq.1.and.nb(20,ki).eq.1)then          ! free CO2
!                                                                                 dxi(ki)=0.19 ! lower EN for O
!                                                                                 dxi(ji)=0.19 !  "    "   "  "
!           endif 
!        endif
         if(ati.eq.6.and.nn.eq.1.and.at(ji).eq.8.and.nb(20,ji).eq.1)              dxi(ji)=0.15! free CO
!     nitrogen
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      write(*,'(10x,"pair mat ...")')
      call nbondmat(n,nb,bpair)  ! get number of cov. bonds between atoms up to 4 bonds
      write(*,'(10x,"computing topology distances matrix with Floyd-Warshall algo ...")')
      allocate( rabd(n,n), source = 0.0e0_sp)
      rabd = 1.0+12           
...
!     first check for fragments
      call mrecgff(n,nbf,nfrag,fraglist)
...
!     read QM info if it exists
      ...
      if(n.lt.100.and.nfrag.gt.2.and.ichrg.ne.0.and.sum(qfrag(2:nfrag)).gt.999) then
         itmp=0
         do i=1,n
      ...
      if(n.ge.100.and.nfrag.gt.2.and.ichrg.ne.0.and.sum(qfrag(2:nfrag)).gt.999) then
         qfrag(1)=ichrg
         qfrag(2:nfrag)=0
      endif
      if(nfrag.eq.2.and.ichrg.ne.0.and.sum(qfrag(2:nfrag)).gt.999) then
         ...
         call goedeckera(n,at,nb,rtmp,qa,dum1)
         qfrag(2)=0; qfrag(1)=dble(ichrg)
         call goedeckera(n,at,nb,rtmp,qa,dum2)
...
!     make estimated, topology only EEQ charges from rabd values, including "right" fragment charge
      call goedeckera(n,at,nb,rtmp,qa,ees)
...
!     estimate how much of the frag charge is on the pi sub systems
      if(picount.gt.0.and.qloop_count.gt.0) then
         ...
         qah  = qa
         call qheavy(n,at,nb,qah) ! heavy atoms only ie H condensed to neighbor
         do pis=1,picount ! loop over pi systems
            do k=1,npiall  
               if(pimvec(k).eq.pis) then
                  kk=piadr(k)
            ...
            call goedeckera(n,at,nb,rtmp,qa,ees) ! for neutral
            qfrag(ifrag) = dum2 ! back
            call qheavy(n,at,nb,qa)   
      ...
      if(qloop_count.eq.0) itmp(1:n)=nb(20,1:n)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do i=1,n 
         ...
         if(at(i).eq. 7)                 then
           ...
           if(amide(n,at,hyb,nb,piadr,i))ff=-0.16    
         endif
         if(at(i).eq. 8)                 then
                                         ff=-0.15 ! O
...
!     prepare true EEQ parameter, they are ATOMIC not element specific!
      do i=1,n
!                   base val   spec. corr. 
         chieeq(i)=-chi(at(i)) + dxi(i)
         gameeq(i)= gam(at(i)) +dgam(i)
         ff = 0
         if(at(i).eq.6)       ff= 0.09
         if(at(i).eq.7)       ff=-0.21
         if(group(at(i)).eq.6)ff=-0.03
         if(group(at(i)).eq.7)ff= 0.50
         if(imetal(i).eq.1)   ff= 0.3 
         if(imetal(i).eq.2)   ff=-0.1 
         alpeeq(i) = (alp(at(i))+ff*qa(i))**2
      enddo
      deallocate(dgam,dxi)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      write(*,'(10x,"rings ...")')
!$omp parallel default(none) private(i,cr,sr) shared(at,n,nbm,cring,sring)
!$omp do 
      do i=1,n
         call getring36(n,at,nbm,i,cr,sr)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do i = 1,n
         ...
         ! Carbonyl R-C=O
         if(ati.eq.8.and.nn.eq.1.and.at(nb(nn,i)).eq.6) hbbas(i) = 0.68 
         ! Nitro R-N=O
         if(ati.eq.8.and.nn.eq.1.and.at(nb(nn,i)).eq.7) hbbas(i) = 0.47
      end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      allocate( hbatHl(n),hbatABl(2,n*(n+1)/2), source = 0 ) 
      ...
      do i=1,n
         ...
         do j=1,i-1
            ...
            if(qa(j).gt.ff) cycle
            call hbonds(i,j,at(i),at(j),hbpi,hbpj)
...
! make ABX list 
      ...
      do i=1,n
         do ia=1,nb(20,i)
            ix=nb(ia,i)
            if(xatom(at(ix))) then
            if(at(ix).eq.16.and.nb(20,ix).gt.2) cycle ! no sulphoxide etc S
            do j=1,n  
               if(i.eq.j.or.j.eq.ix) cycle
               if(bpair(lin(j,ix)).le.3) cycle   ! must be A...B and not X-B i.e. A-X...B
               if(xhbas(at(j)).lt.1.d-6) cycle   ! B must be O,N,...
               if(group(at(j)).eq.4    ) then
      ...
      call gfnff_hbset0(n,at,xyz,sqrab)
...
! iterative Hueckel loop, off-diag terms are reduced depending on P to avoid overdelocalization
      ...
      do i=1,npi
         ii=piadr3(i)
         Api(i,i)=hdiag(at(ii))+qa(ii)*hueckelp3-dble(piel(ii)-1)*pilpf
      enddo
!     loop over bonds for pair interactions
      do i=1,nbond
         ...
         if(ia.gt.0.and.ja.gt.0) then
            ...
            dum=sqrt(hoffdiag(at(ii))*hoffdiag(at(jj)))-dum           ! better than arithmetic
            dum2=hiter
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do i=1,n
         if(hyb(i).eq.2 .and. piadr(i).eq.0 .and. nb(20,i).eq.3 .and. group(at(i)).eq.4)then ! C,Si,Ge... CN=3, no pi
            jj=nb(1,i)
            kk=nb(2,i)
            ll=nb(3,i)
            phi=omega(n,xyz,i,jj,kk,ll)  ! the shitty second geom. dep. term GEODEP
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      call gfnffrab(n,at,cn,rtmp)           ! guess RAB for output

      nbond_vbond = nbond
      allocate( vbond(3,nbond), source = 0.0d0 )

      write(*,*)
      write(*,'(10x,"#atoms :",3x,i0)') n
      ...
      do i=1,nbond
         ...
         ia=at(ii)
         ja=at(jj)
         call ringsbond(n,ii,jj,cring,sring,rings)
         shift =0.d0
         fxh   =1.d0
         ringf =1.d0
         fqq   =1.d0
...
! normal bond 
     ...
     &                                      bstrength=bstren(2)*1.04   ! N-sp2 
            ...
            if( (ia.eq.1.and.ja.eq.6) )then
                           call ringsatom(n,jj,cring,sring,ringsj)
                           if(ringsj.eq.3)                 fxh=1.05    ! 3-ring CH
                           if(ctype(n,at,nb,piadr,jj).eq.1)fxh=0.95    ! aldehyd CH
            endif
            if( (ia.eq.6.and.ja.eq.1) )then
                           call ringsatom(n,ii,cring,sring,ringsi)
                           if(ringsi.eq.3)                 fxh=1.05    ! 3-ring CH
                           if(ctype(n,at,nb,piadr,ii).eq.1)fxh=0.95    ! aldehyd CH
            endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      call bond_hbset0(n,at,xyz,sqrab,bond_hbn)
      allocate(bond_hbl(3,bond_hbn))
      allocate(nr_hb(nbond), source=0)
      call bond_hbset(n,at,xyz,sqrab,bond_hbn,bond_hbl)
      
      !Set up AH, B and nr. of B list
      call bond_hb_AHB_set0(n,at,nbond,bond_hbn,bond_hbl,AHB_nr)
      allocate( lin_AHB(0:AHB_nr), source=0  )
      call bond_hb_AHB_set1(n,at,nbond,bond_hbn,bond_hbl,AHB_nr,lin_AHB,bond_hb_nr,b_max)
      ...
      call bond_hb_AHB_set(n,at,nbond,bond_hbn,bond_hbl,AHB_nr,lin_AHB)
      ...
      do i=1,n
         ...
         do j=1,nn  
            do k=1,j-1 
               ...
               alist(2,nangl)=jj       
               alist(3,nangl)=kk           
               call ringsbend(n,ii,jj,kk,cring,sring,rings)
...
!                 in pi system 
                  if(npi.gt.0)then
                                                                 if(amide(n,at,hyb,nb,piadr,i))then        
                                                                   r0=115.  
                                                                   f2=1.2d0   
                                                                 else
...
! R-X in 3-rings e.g. cyclopropene
               if(rings.eq.0)then
                  call ringsatom(n,i,cring,sring,idum)
                  if(idum.eq.3) then
                     call ringsatom(n,jj,cring,sring,ringsj)
                     call ringsatom(n,kk,cring,sring,ringsk)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do m=1,nbond
         ...
         if(tors(at(ii)).lt.0.or.tors(at(jj)).lt.0) cycle ! no negative values
         if(tors(at(ii))*tors(at(jj)).lt.1.d-3)     cycle ! no small values
         if(metal(at(ii)).gt.1.and.nb(20,ii).gt.4)  cycle ! no HC metals
         if(metal(at(jj)).gt.1.and.nb(20,jj).gt.4)  cycle ! 
      ...
      do m=1,nbond
         ...
         fij=tors(at(ii))*tors(at(jj))             ! atom contribution, central bond
         if(fij.lt.fcthr)                 cycle
         if(tors(at(ii)).lt.0.or.tors(at(jj)).lt.0) cycle ! no negative values
         if(metal(at(ii)).gt.1.and.nb(20,ii).gt.4)  cycle ! no HC metals
         if(metal(at(jj)).gt.1.and.nb(20,jj).gt.4)  cycle ! 
         fqq=1.0d0+abs(qa(ii)*qa(jj))*qfacTOR      ! weaken it for e.g. CF-CF and similar
         call ringsbond(n,ii,jj,cring,sring,rings) ! i and j in same ring
         lring=.false.
         ccij =.false.
         ...
         nhj=1
         do ineig=1,nb(20,ii)
            if(at(nb(ineig,ii)).eq.1) nhi=nhi+1
         enddo
         do jneig=1,nb(20,jj)
            if(at(nb(jneig,jj)).eq.1) nhj=nhj+1
         enddo
...
!        loop over neighbors of ij
         do ineig=1,nb(20,ii)
            ...
            do jneig=1,nb(20,jj)
               ...
               if(ll.eq.ii) cycle
               if(ll.eq.kk) cycle
               if(chktors(n,xyz,ii,jj,kk,ll)) cycle  ! near 180
               fkl=tors2(at(kk))*tors2(at(ll))       ! outer kl term
               if(at(kk).eq.7.and.piadr(kk).eq.0) fkl=tors2(at(kk))*tors2(at(ll))*0.5
               if(at(ll).eq.7.and.piadr(ll).eq.0) fkl=tors2(at(kk))*tors2(at(ll))*0.5
!              if(amide(n,at,hyb,nb,piadr,kk))    fkl=tors2(at(kk))*tors2(at(ll))*1.0
!              if(amide(n,at,hyb,nb,piadr,ll))    fkl=tors2(at(kk))*tors2(at(ll))*1.0
               if(fkl.lt.fcthr)               cycle
               if(tors(at(kk)).lt.0.or.tors(at(ll)).lt.0) cycle ! no negative values
               f1 = torsf(1)
               f2 = 0.0d0
...
!-----------------------
               if ( lring ) then
                    if(rings.gt.3)then
                    call ringstors(n,ii,jj,kk,ll,cring,sring,rings4) ! smallest ring in which i,j,k,l are
                    else
...
! RING CASE    
                    ...
                    if ( btyp(m).eq.1 .and. rings4 .gt. 0 )then
                      call ringstorl(n,ii,jj,kk,ll,cring,sring,ringl)  ! largest ring in which i,j,k,l are
                    ...
                    if(btyp(m).eq.2 .and. rings.eq.5 .and. at(ii)*at(jj).eq.42) then
                       if(amide(n,at,hyb,nb,piadr,ii).or.amide(n,at,hyb,nb,piadr,jj)) f1=5.  ! improving CB7
                    endif
               else
! ACYCLIC      
                                                      phi  = 180.d0 ! trans
...
! N-N, P-P ...
               if(group(at(ii)).eq.5.and.group(at(jj)).eq.5) then
                                                             nrot=3 
                                                             phi=60.d0 
                                                             f1= 3.0d0 
               endif
! 5-6           
               if((group(at(ii)).eq.5.and.group(at(jj)).eq.6).or. &
     &            (group(at(ii)).eq.6.and.group(at(jj)).eq.5))then 
                                                             nrot=2 
                                                             phi=90.d0 
                                                             f1=1.0d0 
...
! O-O, S-S ...
               if(group(at(ii)).eq.6.and.group(at(jj)).eq.6) then
                                                             nrot=2 
                                                             phi=90.d0 
                                                             f1=5.0d0
...
! total FC            sigma       pi             charge central outer kl
               ...
               if(fctot.gt.fcthr) then ! avoid tiny potentials
                  ...
                  vtors(2,ntors)=fctot
!                 printout
                  phi=valijklff(n,xyz,ll,ii,jj,kk)
...
! extra rot=1 torsion potential for sp3-sp3 to get gauche conf energies well
               ...
               if(sp3kl.and.sp3ij.and.(.not.lring).and.btyp(m).lt.5) then
                  ...
                  if(ntors.gt.maxtors) stop 'internal (torsion setup) error'
                  ff = torsf(6)
                  if(at(ii).eq.7.or.at(jj).eq.7) ff = torsf(7)
                  if(at(ii).eq.8.or.at(jj).eq.8) ff = torsf(8)
                  tlist(1,ntors)=ll
                  tlist(2,ntors)=ii
                  tlist(3,ntors)=jj
                  tlist(4,ntors)=kk
...
!        sort atoms according to distance to central atom such that the same inversion angle def. always results
         ...
         if(piadr(i).eq.0.and.at(i).eq.7) then  ! sat N case
           ...
           do m=1,nb(20,i)
              idum=nb(m,i)
              vtors(2,ntors)=vtors(2,ntors)+ff*sqrt(repz(at(idum)))  ! NX3 has higher inv barr. than NH3
           enddo
         else
           ncarbo=0
           nf    =0
...
!          carbonyl corr.
           if(at(i).eq.5.and.ncarbo.gt.0)             vtors(2,ntors)=vtors(2,ntors)*38.      
           if(at(i).eq.6.and.ncarbo.gt.0)             vtors(2,ntors)=vtors(2,ntors)*38.      
           if(at(i).eq.6.and.nf.gt.0.and.ncarbo.eq.0) vtors(2,ntors)=vtors(2,ntors)*10.    
           if(at(i).eq.7.and.ncarbo.gt.0)             vtors(2,ntors)=vtors(2,ntors)*10./f2 ! no pi dep
         endif
!        printout
         phi=omega(n,xyz,i,jj,kk,ll)
...
! all done
      ...
      maxsystem = 5000
      !if(n.gt.500)then
      call fragmentize(n,at,xyz,maxsystem,500,rab,nb,ispinsyst,nspinsyst,nsystem)
      !else
      !   nsystem=1
      !endif
