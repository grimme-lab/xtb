...
To build `xtb` from the source in this repository can be employed using the https://cmake.org/[CMake build system].
The build files used require a CMake version of 3.17 or newer.
...
With CMake 3.17 and newer the configuration can be conveniently performed from the repository root by
...
The configuration with CMake 3.17 or newer can be performed conveniently with
...
== Profiling xtb with Tracy

`xtb` can be profiled using the in-source https://github.com/wolfpld/tracy[Tracy] profiler.
To enable this feature, add the extra `-DWITH_TRACY=ON` flag during configuration:

[source,bash]
----
cmake -B_build -S . -GNinja -DCMAKE_BUILD_TYPE=Release -DWITH_TRACY=ON
----

Compile xtb as usual. The resulting build will act as a Tracy client.
Note that it will be configured with the latest commit of Tracy, so you must also fetch the latest Tracy sources and compile the Tracy GUI server.
For detailed instructions on building the server, refer to the Tracy repository and its documentation.

Once the server is running, you can start profiling `xtb`.
`xtb` is configured to wait for an incoming connection from the server before beginning a calculation.
After the connection is established, you will see the running `xtb` job in the console along with real-time visual updates in the Tracy GUI showing which parts of the code are being executed.

=== Profiling on a remote server

If you want to profile `xtb` running on a remote machine, select a port to use -- for example, `4444`.
Then, create an SSH tunnel with port forwarding:
...
This makes the remote server’s port `4444` accessible on your local machine.
In the Tracy GUI, set the IP address to `127.0.0.1` and the Port to `4444` to connect to the remotely running instance of `xtb`.
