...
module relaxation_engine
   use iso_fortran_env, only : wp => real64, rfp => real32
...
subroutine lbfgs_relax &
   ...
   real(rfp) :: dsnrm
   real(rfp), external :: sdot
   real(rfp), allocatable :: eaug(:)
   real(rfp), allocatable :: Uaug(:,:)
   real(rfp), allocatable :: Aaug(:)
   ...
   allocate(Uaug(nvar1,1),eaug(nvar1),Aaug(npvar1), source = 0.0_rfp)
   ...
   if (abs(Uaug(nvar1,1)) .lt. 1.0e-10_rfp) error stop "Internal RF error"
