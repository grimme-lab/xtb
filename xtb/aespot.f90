   ...
   do iat=1,nat
      ...
      do jat=1,iat
         ...
         do ish=1,gfn%ao_n(iatyp)
            ...
            naoi=llao(ishtyp)
            iptyp=itt(ishtyp)
            jshmax=gfn%ao_n(jatyp)
            if(iat.eq.jat) jshmax=ish
            !              jshells
            do jsh=1,jshmax
               jshtyp=gfn%ao_l(jsh,jatyp)
...
subroutine aniso_electro(nat,at,xyz,q,dipm,qp,gab3,gab5,e,epol)
   ...
   do i=1,nat
      ...
      do k=1,3
         ...
         enddo
      enddo
      epol=epol+gfn%dpolc(at(i))*tt+tt3*gfn%qpolc(at(i))
      ! ---
...
subroutine setvsdq(nat,at,xyz,q,dipm,qp,gab3,gab5,vs,vd,vq)
   ...
   do i=1,nat
      ...
      ! --- CT correction terms
      qs1=gfn%dpolc(at(i))*2.0_wp
      qs2=gfn%qpolc(at(i))*6.0_wp ! qpole pot prefactors
      t3a=0.0_wp
      t2a=0.0_wp
      do l1=1,3
         ! potential from dipoles
...
!$omp&         shared(s,cont,alp,saoshell,caoshell,thr2,primcount,nprim, &
!$omp&                itt,xyz,at,nat,gfn)
!$omp do schedule(dynamic)
   do iat=1,nat
      ra(1:3)=xyz(1:3,iat)
      iatyp=at(iat)
      !         write (*,*), 'i',iat, iatyp, gfn%ao_n(iatyp)
      do jat=1,iat
         rb(1:3)=xyz(1:3,jat)
         jatyp=at(jat)
         dx=ra(1)-rb(1)
         ...
         if(rab2.gt.2000) cycle
         !            write(*,*) 'j',jat,jatyp,gfn%ao_n(jatyp)
         do ish=1,gfn%ao_n(iatyp)
            ishtyp=gfn%ao_l(ish,iatyp)
...
subroutine get_radcn(n,at,cn,shift,expo,rmax,radcn)
   ...
   do i=1,n
      ati = at(i)
      rco=gfn%radaes(ati)             ! base radius of element
      t1 =cn(i)-gfn%cnval(ati)-shift  ! CN - VALCN - SHIFT
...
subroutine dradcn(n,at,cn,shift,expo,rmax,dcn)
   ...
   do i=1,n
      ati = at(i)
      rco=gfn%radaes(ati)             ! base radius of element
      t1 =exp(-expo*(cn(i)-gfn%cnval(ati)-shift))  ! CN - VALCN - SHIFT
      t2 =(rmax-rco)/(1.0_wp+2.0_wp*t1+t1*t1)
      t2 = t2*expo*t1
      do j=1,i
         atj = at(j)
         rco=gfn%radaes(atj)             ! base radius of element
         t3 =exp(-expo*(cn(j)-gfn%cnval(atj)-shift))  ! CN - VALCN - SHIFT
