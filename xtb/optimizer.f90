...
subroutine relax(iunit,iter,mol,anc,restart,maxcycle,maxdispl,ethr,gthr, &
   ...
   real(real32),allocatable :: eaug(:)
   real(real32),allocatable :: Uaug(:,:)
   real(real32),allocatable :: Aaug(:)
   real(real32) :: r4dum,sdot
...
!  chose your favourite solver
   if (exact .or. nvar1.lt.50) then
      ...
      if (ii.eq.1) then
         Uaug(:,1)=[-real(gint(1:anc%nvar),real32),1.0_real32]
