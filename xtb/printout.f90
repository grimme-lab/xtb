...
subroutine prelemparam(globpar)
   ...
   do i = 1, max_elem
      write(id,'("data atomparameter(",i0,") / tb_parameter( & !",1x,a)') &
         &  i,asym(i)
      call prelemdata_inc(id,'en',gfn%en(i))
      call prelemdata_inc(id,'mc',gfn%mc(i))
      call prelemdata_inc(id,'gam',gfn%gam(i))
      call prelemdata_inc(id,'gam3',gfn%gam3(i))
      call prelemdata_inc(id,'wll',gfn%wll(i,:))
      call prelemdata_inc(id,'rep',gfn%rep(:,i))
      call prelemdata_inc(id,'polyr',gfn%polyr(:,i))
      call prelemdata_inc(id,'cxb',gfn%cxb(i))
      call prelemdata_inc(id,'ao_exp',gfn%ao_exp(:,i))
      call prelemdata_inc(id,'ao_lev',gfn%ao_lev(:,i))
      call prelemdata_inc(id,'lpar',gfn%lpar(:,i))
      call prelemdata_inc(id,'kcnat',gfn%kcnat(:,i))
      call prelemdata_inc(id,'radaes',gfn%radaes(i))
      call prelemdata_inc(id,'dpolc',gfn%dpolc(i))
      call prelemdata_inc(id,'qpolc',gfn%qpolc(i))
      call prelemdata_inc(id,'ao_pqn',gfn%ao_pqn(:,i))
      call prelemdata_inc(id,'ao_l',gfn%ao_l(:,i))
      call prelemdata_inc(id,'ao_n',gfn%ao_n(i))
      call prelemdata_inc(id,'ao_typ',gfn%ao_typ(:,i))
      call prelemdata_inc(id,'cnval',gfn%cnval(i))
      ! print at the end, because it closes the parameter block (this is a hack)
      call prelemdata_inc(id,'timestp',gfn%timestp(i))
   enddo
   ...
   write(id,'("gfn%kpair = 1.0_wp")')
   do i = 1, max_elem
      do j = 1, max_elem
         if (abs(gfn%kpair(j,i)-1.0_wp).gt.1.0e-5_wp) then
   ...
   write(id,'("gfn%en(1:94)          = atomparameter % en")')
   write(id,'("gfn%mc(1:94)          = atomparameter % mc")')
   write(id,'("gfn%gam(1:94)         = atomparameter % gam")')
   write(id,'("gfn%gam3(1:94)        = atomparameter % gam3")')
   write(id,'("do i = 1,10")')
   write(id,'("   gfn%wll(1:94,i)    = atomparameter % wll(i)")')
   write(id,'("enddo")')
   write(id,'("gfn%rep(1,1:94)       = atomparameter % rep(1)")')
   write(id,'("gfn%rep(2,1:94)       = atomparameter % rep(2)")')
   write(id,'("do i = 1,4")')
   write(id,'("   gfn%polyr(i,1:94)   = atomparameter % polyr(i)")')
   write(id,'("enddo")')
   write(id,'("gfn%cxb(1:94)         = atomparameter % cxb")')
   write(id,'("do i = 1,10")')
   write(id,'("   gfn%ao_exp(i,1:94) = atomparameter % ao_exp(i)")')
   write(id,'("   gfn%ao_lev(i,1:94) = atomparameter % ao_lev(i)")')
   write(id,'("   gfn%ao_pqn(i,1:94) = atomparameter % ao_pqn(i)")')
   write(id,'("   gfn%ao_l(i,1:94)   = atomparameter % ao_l(i)")')
   write(id,'("   gfn%ao_typ(i,1:94) = atomparameter % ao_typ(i)")')
   write(id,'("enddo")')
   write(id,'("do i = 0,2")')
   write(id,'("   gfn%lpar(i,1:94)   = atomparameter % lpar(i)")')
   write(id,'("   gfn%kcnat(i,1:94)  = atomparameter % kcnat(i)")')
   write(id,'("enddo")')
   write(id,'("gfn%radaes(1:94)      = atomparameter % radaes")')
   write(id,'("gfn%dpolc(1:94)       = atomparameter % dpolc")')
   write(id,'("gfn%qpolc(1:94)       = atomparameter % qpolc")')
   write(id,'("gfn%ao_n(1:94)        = atomparameter % ao_n")')
   write(id,'("gfn%cnval(1:94)       = atomparameter % cnval")')
   write(id,'("gfn%timestp(1:94)     = atomparameter % timestp")')
